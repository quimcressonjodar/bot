...
class ClanClient:
    ...
    async def get_clan_data(self, clan_name: str) -> dict[str, Any]:
        ...
        return clan_data
        def generate_leaderboard_image(clans, page, total_pages):
    width = 900
    row_height = 55
    header_height = 80

    height = header_height + len(clans) * row_height + 80
    ...
    # TITLE
    draw.text((20, 20), "🏆 GLOBAL TOP CLANS", fill=(255, 215, 0), font=font_title)
    draw.text((20, 45), f"Page {page}/{total_pages}", fill=(180, 180, 180), font=font)

    # HEADER
    y = header_height
    draw.rectangle([(20, y-10), (880, y+30)], fill=(40, 40, 60))

    draw.text((30, y), "Rank", fill=(255,255,255), font=font)
    draw.text((120, y), "Clan", fill=(255,255,255), font=font)
    draw.text((400, y), "Members", fill=(255,255,255), font=font)
    draw.text((550, y), "Scores", fill=(255,255,255), font=font)
    ...
    for i, c in enumerate(clans):
        ...
        # COLORS
        bg = (30, 30, 45) if i % 2 == 0 else (25, 25, 40)
        ...
        draw.rectangle([(20, y-10), (880, y+40)], fill=bg)
        ...
        draw.text((30, y), medal, fill=(255,255,255), font=font)
        draw.text((120, y), name, fill=(255,255,255), font=font)
        draw.text((400, y), str(members), fill=(200,200,200), font=font)
        draw.text((550, y), scores, fill=(200,200,200), font=font)
    ...
    async def update(self, interaction):
        ...
        image = generate_leaderboard_image(page_data, self.page, total_pages)

        file = discord.File(fp=image, filename="leaderboard.png")

        await interaction.response.edit_message(
            content=f"🏆 Global Top Clans (Page {self.page}/{total_pages})",
            attachments=[file],
            view=self
        )
...
def validate_environment() -> None:
    ...
    if not KIRKA_API_KEY:
        ...
        @bot.tree.command(name="top_clans", description="Global clans leaderboard (image + buttons)")
async def top_clans(interaction: discord.Interaction):
    ...
    try:
        ...
        view = TopClansView(results)

        page_data = view.get_page_data()
        total_pages = view.max_pages()

        image = generate_leaderboard_image(page_data, 1, total_pages)

        file = discord.File(fp=image, filename="leaderboard.png")

        await interaction.followup.send(
            content=f"🏆 Global Top Clans (Page 1/{total_pages})",
            file=file,
            view=view
        )
