...
# Format: {symbol: [list of news tuples (message, price_multiplier)]}
# Multiplier > 1.0 is positive, < 1.0 is negative
...
def get_random_news():
    """Get a random news event from the database."""
    # 70% chance of company-specific news, 30% chance of general news
    if random.random() < 0.7:
        symbol = random.choice(list(MARKET_NEWS.keys())[:-1]) # Exclude GENERAL
